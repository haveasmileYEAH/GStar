...
# -*- coding: utf-8 -*-
"""
GStar A* Retrieval Prototype (self-contained)
- Build a memory graph from text chunks using TF-IDF (English)
- Add KNN similarity edges (cost = 1 - cosine_sim) + temporal edges (small cost)
- A* search with:
    g(n): cumulative estimated token cost + repetition penalty
    h(n): 1 - cosine_sim(query, node)
- Stop when evidence coverage (sum of sims on path) >= tau, or when budget_nodes reached
Optional: if OPENAI_API_BASE and OPENAI_API_KEY are set, ask an LLM (e.g., Qwen Intl) to answer with evidence.
"""
...
def est_tokens(s: str) -> int:
    ...
    # ~4 chars per token or 1.3x words
    w = len(s.split())
...
class MemoryGraph:
    def __init__(self, texts: List[str], meta: List[Dict[str, Any]]):
        ...
        self.doc_mat = self.vectorizer.fit_transform(texts)  # (N, V)
        self.adj = [[] for _ in range(self.N)]  # list of (j, cost)
    ...
    def build_knn_edges(self, k: int = 8, sim_th: float = 0.20):
        ...
        for i in range(self.N):
            ...
            kk = min(k, self.N-1)
            order = np.argpartition(-sims[i], kk)[:kk+1]
            for j in order:
                if j == i: 
                    continue
                if sims[i, j] >= sim_th:
                    cost = 1.0 - float(sims[i, j])
                    self.adj[i].append((j, cost))

    def build_temporal_edges(self, alpha: float = 0.15):
        # connect consecutive items (meta must have "t")
        order = sorted(range(self.N), key=lambda x: self.meta[x].get("t", x))
        for a, b in zip(order, order[1:]):
            self.adj[a].append((b, alpha))  # small cost to follow time
...
def astar_search(graph: MemoryGraph, q_vec, budget_nodes: int = 6, tau: float = 1.6,
    ...
    def h(node_idx: int) -> float:
        # admissible-ish heuristic: 1 - sim to query (in [0,1])
        return max(0.0, 1.0 - float(sims[node_idx]))

    # (f, g, node, path, used_texts_set, sum_sims, total_tokens)
    used_idxs = set()
    ...
    while openpq:
        ...
        # termination: evidence coverage or path length
        ...
        for v, edge_cost in graph.adj[u]:
            # repetition penalty if v already used (shouldn't happen as we track set)
            rep_pen = rep_lambda if v in used else 0.0
            add_tokens = est_tokens(graph.texts[v])
            g2 = g + edge_cost + rep_pen + 0.01 * add_tokens  # add small token scaling inside g
...
def main():
    ...
    if args.data and os.path.exists(args.data):
        ...
        # toy corpus (ALFWorld-like memo snippets)
        toy = [
