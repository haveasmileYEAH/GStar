...
class GPTWrapper:
    ...
    def __call__(self, prompt: str, stop: List[str] = None, max_tokens: int = 256, mode: str = 'chat', model=None,
                 sys_msg=None, use_json=False) -> str:
        if not model:
            model = self.model
        try:
            cur_try = 0
            while cur_try < 6:
                if mode == "chat":
                    text = self.get_chat(prompt=prompt, model=model, temperature=cur_try * 0.2, stop_strs=stop,
                                         max_tokens=max_tokens, sys_msg=sys_msg, use_json=use_json)
                elif mode == "complete":
                    text = self.get_completion(prompt=prompt, model=model, temperature=cur_try * 0.2, stop_strs=stop,
